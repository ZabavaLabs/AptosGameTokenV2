...
module dfa::armor {
    ...
    /// The iron token name
    const IRON_ARMOR_TOKEN_NAME: vector<u8> = b"Iron Armor Token";
    /// The leather token name
    const LEATHER_ARMOR_TOKEN_NAME: vector<u8> = b"Leather Armor Token";
    ...
    public entry fun mint_equipment(creator: &signer, receiver: address, 
        collection_name: String, equipment_token_name: String, amount: u64) acquires ArmorToken {
        let equipment_token = object::address_to_object<ArmorToken>(equipment_token_address(
            collection_name, 
            equipment_token_name
        ));
        mint_internal(creator, equipment_token, receiver, amount);
    }

    /// Transfers the given amount of the leather token from the given sender to the given receiver.
    public entry fun transfer_equipment(from: &signer, to: address, collection_name: String, equipment_token_name: String, amount: u64) {
        transfer_armor(from, object::address_to_object<ArmorToken>(
            equipment_token_address(collection_name, equipment_token_name)
        ), to, amount);
    }
    ...
    public fun test_armor(creator: &signer, user1: &signer, user2: &signer) acquires ArmorToken {
        ...
        mint_equipment(user1, user1_addr, string::utf8(ARMOR_COLLECTION_NAME), string::utf8(IRON_ARMOR_TOKEN_NAME) , 100);
        mint_equipment(user2, user2_addr, string::utf8(ARMOR_COLLECTION_NAME), string::utf8(IRON_ARMOR_TOKEN_NAME) ,200);
