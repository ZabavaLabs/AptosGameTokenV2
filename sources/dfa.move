...
public entry fun create_armor_type(creator: &signer, armor_type: String) {
 ...
 inline fun authorized_borrow<T: key>(creator: &signer, token: &Object<T>): &ArmorToken {
    ...
    public fun test_armor(creator: &signer, user1: &signer, user2: &signer) acquires ArmorToken {
        ...
        mint_armor(creator, armor_token_address(string::utf8(b"Armor"),),user1_addr, 100);

        let armor_token = object::address_to_object<ArmorToken>(armor_token_address(string::utf8(b"Armor")));
